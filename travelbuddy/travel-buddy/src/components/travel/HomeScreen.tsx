...
function PlanItineraryView() {
  ...
  useEffect(() => { setHistory(readPlanHistory()); }, [trip?.id]);
  ...
  if (!ready) return <TravelShell><BrandLoader message="Loading your plan" /></TravelShell>;
      ...
      <TravelShell>
        <div className="px-5 pb-32">
          <div className="pt-2 pb-6">
            <p className="text-[10.5px] font-bold uppercase tracking-[0.11em] text-black/35 mb-2">Your plan</p>
            <h1 className="text-[30px] font-bold tracking-[-0.02em] text-black leading-[1.08]">Nothing planned yet.</h1>
            <p className="mt-2 text-[14px] leading-relaxed text-black/45">
              Answer a few questions and swipe through Goa, and your day-by-day plan appears here.
            </p>
          </div>
          <Link href="/plan" className="flex h-[52px] w-full items-center justify-center gap-2 rounded-full bg-[#FFD233] text-[16px] font-bold text-black shadow-[0_8px_24px_rgba(255,210,51,0.35)]">
            Start planning <ArrowRight size={18} />
          </Link>
          {history.length > 0 && <PlanHistory history={history} setHistory={setHistory} />}
        </div>
      </TravelShell>
    );
  }
      ...
      {confirmDelete && (
        <div className="fixed inset-y-0 left-1/2 z-50 flex w-full max-w-[448px] -translate-x-1/2 items-end bg-black/40">
          <div className="w-full rounded-t-3xl bg-white p-6 pb-10">
            ...
            <button type="button" onClick={deleteCurrentPlan}
              className="mt-5 w-full rounded-full bg-[#E9633B] py-4 text-[15px] font-bold text-white">Delete plan</button>
            <button type="button" onClick={() => setConfirmDelete(false)}
              className="mt-2 w-full py-3.5 text-[14px] font-semibold text-[#8E8E93]">Keep it</button>
          </div>
        </div>
      )}
...
function PlanHistory({ history, setHistory }: { history: ArchivedPlan[]; setHistory: (h: ArchivedPlan[]) => void }) {
  ...
  return (
    <div className="mt-8">
      ...
      <div className="mt-3 flex flex-col gap-2.5">
        {history.map((p) => (
          <div key={p.id + p.archivedAt} className="flex items-center gap-3 rounded-2xl bg-white p-2.5 shadow-[0_1px_8px_rgba(0,0,0,0.05)]">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src={p.image || "/goa/beaches-of-goa.jpg"} alt="" className="h-14 w-14 flex-none rounded-xl object-cover" />
            <div className="min-w-0 flex-1">
              <p className="truncate text-[14px] font-bold text-black">{p.name}</p>
              <p className="mt-0.5 text-[11.5px] text-black/45">
                {p.dayCount} days · ~₹{p.estimatedCost.toLocaleString("en-IN")}{p.bookedAt ? " · confirmed" : ""}
              </p>
              <p className="text-[10.5px] text-black/30">Archived {formatArchivedDate(p.archivedAt)}</p>
            </div>
